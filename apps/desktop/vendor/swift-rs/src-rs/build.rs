...
fn globalize_cdecl_symbols(archive: &std::path::Path, _package_name: &str) {
    if !archive.exists() {
        return;
    }
    ...
    // VENDORED CHANGE: every archive member is considered, not only the one
    // named after the package. The package's Swift dependencies (SwiftRs.o
    // in Tauri's iOS API) export @_cdecl symbols the Rust side links
    // against, and Xcode 27 marks those local as well.
    ...
    for line in String::from_utf8_lossy(&nm.stdout).lines() {
        if line.ends_with(':') {
            ...
            if member.strip_suffix(".o").is_some() {
                continue;
            }
        }
