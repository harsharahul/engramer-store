...
pub fn create_delegate_class() {
  ...
  extern "C" fn configuration_for_connecting_scene_session(
    ...
    unsafe {
      ...
      // VENDORED CHANGE: the configuration is returned to UIKit, which
      // retains it after this call. Handing back the pointer of a
      // `Retained` that is dropped here frees the object first, and a
      // release build then crashes in objc_retain while the scene
      // connects. Autoreleasing keeps it alive until the pool drains.
