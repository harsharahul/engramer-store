...
[![Mac app](https://img.shields.io/badge/mac%20app-download%20dmg-111111?logo=apple)](https://github.com/harsharahul/engramer-store/releases)
...
- **Scene labels as tags.** With meaning search on, each photo's vector is scored on the device against a small vocabulary (beach, food, dog, document, screenshot and more) and the confident labels become ordinary tags: searchable, shown as chips, usable in albums. Files that arrive bare from the iPhone's Files app are filled in by whichever device opens next, in one pass per file, narrated through the toolbar's Activity button where it can be stopped. See [docs/intelligence.md](docs/intelligence.md).
- **Search in plain words, on the device.** In the Mac and iPhone apps on macOS 26 and iOS 26, "receipts from last december" is read by Apple's on-device language model into the filters search already understands, and the ordinary engine runs it. The model is given only the library's own vocabulary and keeps nothing; anything it invents is dropped before it reaches the search, the line beside the results shows what was actually searched, and the words as typed stay one click away. Nothing leaves the device. See [docs/intelligence.md](docs/intelligence.md).
- **A one-line summary of every document, on the device.** On the same apps, Apple's on-device model reads the opening pages of each text-bearing document and writes one sentence and a few tags into its encrypted metadata, so every device shows them and search finds a scan by what it is about. Reference numbers are masked before anything is stored, the sentence is capped, and a document the model declines is recorded as such rather than retried. See [docs/intelligence.md](docs/intelligence.md).
- **Ask your files, on the device.** Type a question into the search field on those apps and a row offers to answer it from your own documents: what is already known about every file picks a handful of sources, short excerpts are cut around the words asked about, and Apple's on-device model answers only from those, naming the files, above results that stay exactly where they were. Nothing about the question or the answer is stored or leaves the device. See [docs/intelligence.md](docs/intelligence.md).
...
- **A Finder drive on the Mac.** The Mac app puts the vault under Locations in Finder's sidebar, beside iCloud Drive. Files download and decrypt as they are opened, new and edited files encrypt and upload in place, deletions go to the vault's trash, and a conflicting save becomes a "(conflicted copy)" rather than lost work. Changes from your other devices reach the drive live: the app holds a server change feed that carries only a sync counter, so a phone upload appears in Finder within seconds even while the app sits in the tray, and Profile shows the connection's actual state. Right-clicking a drive file copies a working share link. The app ships as a notarized DMG that asks for your server on first launch, so one download serves any deployment; get it from the [releases page](https://github.com/harsharahul/engramer-store/releases). See [docs/native-apps.md](docs/native-apps.md).
...
- **Quotas and delta sync.** Per-user storage quotas enforced during streaming upload, and a sequence-number sync protocol so clients converge in one round trip. A server-sent change feed pushes the sequence number the moment it moves (including to collaborators), so always-on clients pull immediately instead of polling; the events carry no content, and `ENGRAMER_EVENTS=off` turns the feed off.
- **Registration policy and an admin panel.** Registration can be `open`, `invite`, or `closed`; admins named by environment variable get an inline panel to list accounts, mint and revoke invites, override quotas, and disable or delete accounts. There is deliberately no password reset, because the server never holds key material; the recovery key is the only way back in. Anyone can delete their own account from Profile: the password is proved again on the device (and a second factor when one is on), then every row and every blob of the account is removed for good. Metadata lives in SQLite by default or PostgreSQL for replicated deployments. See [docs/auth.md](docs/auth.md).
...
| `ENGRAMER_EVENTS` | `on` | `off` disables the change feed; always-on clients fall back to their sync cycle |
