...
Format ambitions are deliberately sequenced: Markdown and rich text first, which is the same fidelity envelope Proton Docs launched with; DOCX import and export through client-side conversion next; spreadsheets last. Full-fidelity XLSX/PPTX editing is the one capability that stays with server-side suites, and any future integration of one would be an explicitly labeled opt-in that marks affected files as not end-to-end encrypted, never a silent default.
